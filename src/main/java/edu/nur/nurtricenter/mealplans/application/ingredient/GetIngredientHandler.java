...
@Component
public class GetIngredientHandler implements Command.Handler<GetIngredientCommand, ResultWithValue<IngredientDto>> {
    ...
    public ResultWithValue<IngredientDto> handle(GetIngredientCommand command) {
        if (!repository.existById(command.id())) {
            return ResultWithValue.validationFailure(Error.notFound("NotFound", "Not found ingredient", command.id().toString()));
        }
