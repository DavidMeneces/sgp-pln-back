...
@Component
public class CreateIngredientHandler implements Command.Handler<CreateIngredientCommand, ResultWithValue<UUID>> {
    ...
    public ResultWithValue<UUID> handle(CreateIngredientCommand command) {
        var ingrediente = Ingredient.create(UUID.randomUUID(),
                command.name(), command.description(), command.unitMeasure(), command.caloriesPerGram());
