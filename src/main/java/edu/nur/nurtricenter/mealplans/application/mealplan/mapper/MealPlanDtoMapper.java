...
public final class MealPlanDtoMapper {
	...
	public static MealPlanDto mapper(MealPlan mealPlan) {
		return MealPlanDto.builder()
			...
			.transaction(mealPlan.getTransaction())
			.status(mealPlan.getStatus())
			.createBy(mealPlan.getCreateBy())
			.createAt(mealPlan.getCreateAt())
			.build();
	}

	private static List<MealPlanDayDto> mapperMealPlanDays(List<MealPlanDay> mealPlanDays) {
		return mealPlanDays != null
			? mealPlanDays.stream()
			.map(
				mealPlanDay ->
					MealPlanDayDto.builder()
						.id(mealPlanDay.getId())
						.day(mealPlanDay.getDay())
						.timeFoods(
							mapperTimeFoods(mealPlanDay.getTimeFoods()))
						.build())
			.toList()
			: null;
	}
