...
@Component
public class CreateMealPlanHandler implements Command.Handler<CreateMealPlanCommand, ResultWithValue<UUID>> {
    ...
    public ResultWithValue<UUID> handle(CreateMealPlanCommand command) {
        ...
        try {
            if (!nutricionistModelRepository.existsById(command.idNutricionist())) {
                return ResultWithValue.validationFailure(Error.notFound("NotFound", "Not found nutricionist", command.idNutricionist().toString()));
            }
        ...
        mealPlan.addDomainEvent(new MealPlanCreatedEvent(mealPlan.getId(),
    ...
    private List<MealPlanDay> buildMealPlanDay(List<CreateMealPlanCommand.MealPlanDayCommad> requestList) {
        return (requestList != null) ? requestList.stream()
                .map(request -> MealPlanDay.create(UUID.randomUUID(), request.day(), buildTimeFood(request.timeFoods())))
                .toList() : new ArrayList<>();
    }

    private List<TimeFood> buildTimeFood(List<CreateMealPlanCommand.TimeFoodCommad> requestList) {
        return (requestList != null) ? requestList.stream()
                .map(request -> TimeFood.create(UUID.randomUUID(), request.type(), request.order(),
                        buildTimeFoodRecipe(request.recipes())))
                .toList() : new ArrayList<>();
    }

    private List<TimeFoodRecipe> buildTimeFoodRecipe(List<CreateMealPlanCommand.TimeFoodRecipeCommand> requestList) {
        return (requestList != null) ? requestList.stream()
                .map(request -> {
                    if (!recipeRepository.existById(request.idRecipe())) {
                        throw new DomainException(Error.notFound("NotFound", "Not found recipe", request.idRecipe().toString()));
                    }
                    return TimeFoodRecipe.create(UUID.randomUUID(), request.idRecipe(), request.portion());
                })
                .toList() : new ArrayList<>();
    }
