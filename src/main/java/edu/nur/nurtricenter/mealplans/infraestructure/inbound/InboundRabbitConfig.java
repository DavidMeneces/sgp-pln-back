...
public class InboundRabbitConfig {
	...
	public SimpleRabbitListenerContainerFactory rabbitListenerContainerFactory(
		...
		if (props.isEnabled() && !queueExists) {
			log.warn(
					"Inbound listener disabled because queue '{}' does not exist yet. Provision queue and restart service.",
					props.getQueue());
		}
	...
	public Declarables inboundRabbitDeclarables(InboundSubscriptionProperties props) {
		if (!props.isEnabled()
				|| !props.isDeclareTopology()
				|| props.getQueue() == null
				|| props.getQueue().isBlank()
				|| props.getExchange() == null
				|| props.getExchange().isBlank()) {
	...
	private Exchange buildExchange(InboundSubscriptionProperties props) {
		String type =
				props.getExchangeType() != null
						? props.getExchangeType().trim().toLowerCase()
						: "topic";
