...
public class UnitOfWork implements IUnitOfWork {
	...
	public UnitOfWork(
			EntityManager em, OutboxEventRepository outboxRepository, ObjectMapper objectMapper) {
		this.em = em;
	...
	public CompletableFuture<Void> commitAsync(Entity... entities) {
		...
		List<DomainEvent> domainEvents =
				Arrays.stream(entities).toList().stream()
		...
		if (!domainEvents.isEmpty()) {
			List<OutboxEventEntity> outboxEventEntities =
					domainEvents.stream().map(mapper::toEntity).toList();
