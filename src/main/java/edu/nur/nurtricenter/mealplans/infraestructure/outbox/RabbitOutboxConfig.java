...
public class RabbitOutboxConfig {
	...
	public RabbitTemplate rabbitTemplate(
			ConnectionFactory connectionFactory, OutboxPublisherProperties props) {
	...
	private Exchange buildExchange(OutboxPublisherProperties props) {
		String type =
				props.getExchangeType() != null ? props.getExchangeType().toLowerCase() : "fanout";
		return switch (type) {
			case "topic" -> ExchangeBuilder.topicExchange(props.getExchange())
					.durable(props.isExchangeDurable())
					.build();
			case "direct" -> ExchangeBuilder.directExchange(props.getExchange())
					.durable(props.isExchangeDurable())
					.build();
			case "fanout" -> ExchangeBuilder.fanoutExchange(props.getExchange())
					.durable(props.isExchangeDurable())
					.build();
			default -> ExchangeBuilder.directExchange(props.getExchange())
					.durable(props.isExchangeDurable())
					.build();
		};
	}
	...
	private Binding buildBinding(Queue queue, Exchange exchange, String bindingKey) {
		...
		if (exchange instanceof TopicExchange topicExchange) {
			return BindingBuilder.bind(queue)
					.to(topicExchange)
					.with(bindingKey != null ? bindingKey : queue.getName());
		}
		...
		return BindingBuilder.bind(queue)
				.to(exchange)
				.with(bindingKey != null ? bindingKey : "")
				.noargs();
	}
